...
MoE（Mix of Experts），应用前提把各个领域的专家集合起来一起攻克一个包括了多个领域知识的复杂问题。MoE 模型由多个专业化的子模型即“专家”组合而成，每一个“专家”都在其擅长的领域内做出贡献。我们借鉴了MoE混合专家架构的思路，为不同能力的子模型指派角色，分工合作，对外表现为更强能力的智能体。

![类 MoE 混合专家架构示意图](index/类MoE混合专家架构.png)

### 思维链推理

思维链推理系统，让模型能够模仿学习人类链式思维方式并可视化展现。通过思维链的搭建，模型能够自由决定下一步进行推理、查询还是输出，从而能够尝试不同的推理组合策略，提升模型的解释和深度思考能力，经过多轮次的迭代，模型能输出高质量且条理清晰易于理解的回答内容。此外，我们还通过流式输出提高模型的响应速度和用户的使用体验。

![思维链推理概要示意图](index/思维链概要.png)
...
在“核心-细胞知识库”体系中，我们为给大模型的知识供给增加了相应的权重设置。核心知识库总是会以高优先级提供，而细分领域的细胞知识库则会被划分权重。在每轮对话的 RAG 检索开始前，由 Observer 子模型分析对话中用户的问题与不同领域的相关度并产生权重得分，子模型产生的得分会在规范化后与k值叠加，这样也可以有效解决因术语相似导致的模型对不同领域知识的混淆与误解，减少“幻觉”现象的发生。
...
受到近来涌现的 MoE 架构大模型的启发，我们基于 LangChain 中“链”的概念把多个不同的模型工作流串联起来，使得不同特点的模型可以基于其各自的角色发挥不同的效果，组成子模型协作系统，对外表现出 1+1+1>3 的智能体能力，有效弥补了小尺寸大语言模型的能力缺陷。

![LangChain 链式结构示意图](index/LangChain链式结构.png)

受启发自 Android 架构设计中的 HAL 硬件抽象层，我们将不同的子模型抽象为类似 HAL 硬件抽象层的统一接口，这使我们可以在不同的硬件上运行不同的子模型，从而实现了模型的高度可扩展性。

这种高度抽象的架构设计使得我们可以在不同的基底模型上快速迁移，便于验证不同模型能力差异以及后续进一步的迭代开发。

### 多轮次思维链推理系统

我们通过提示词工程对各个专家模型进行微调，让其接受用户输入以后能够将推理、查询知识库和输出这三种选择可重复地组合，自主决定模型推理策略，在多轮次重复推理下，模型思维能力在多轮推理中得到提高。具体而言，模型能够自主决定下一步的操作，并将指令以 JSON 格式提供，流转至其他模块进一步处理，在这个过程中合法判定模块会对 JSON 输出进行规范化约束，模糊解析功能让模型在指令集不够清晰的情况下依然能作出正确反应。我们还为 JSON 结构化输出实现了流式传输，这意味着模型能够在执行推理任务的同时输出已经生成的部分回答，从而能够提升模型的响应速度，增强用户的使用体验。同时，思维链能够通过联动系统其他模块，如核心细胞知识库和信息处理系统，最大程度发挥 MoE 混合专家架构的优势，使得输出答案更加准确和全面。此外，思维链能将推理过程可视化展现，提高模型推理的逻辑性和可解释性。

![多轮次思维链推理系统示意图](index/思维链.png)
